/**
 * Students Import/Export Module
 */
function initStudentsImportExport() {
    (function($) {
        'use strict';

        const uploadArea = $('.cbedu-students-upload-area');
        const fileInput = $('#cbedu_students_import_file');
        const uploadLabel = uploadArea.find('.cbedu-ie-upload-label');
        const exportForm = $('.cbedu-students-export-form');
        const importForm = $('.cbedu-students-import-form');

        // Initialize drag and drop
        if (uploadArea.length > 0 && fileInput.length > 0) {
            initStudentsDragDrop();
        }

        // Export AJAX handler
        if (exportForm.length > 0) {
            exportForm.on('submit', handleStudentsExport);
        }

        // Import AJAX handler
        if (importForm.length > 0) {
            importForm.on('submit', handleStudentsImport);
        }

        /**
         * Initialize drag and drop for students
         */
        function initStudentsDragDrop() {
         ...
         */
        function handleStudentsExport(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cbedu_export_students',
                    nonce: cbeduImportExport.exportStudentsNonce
                },
                success: function(response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            ...
                            html: `<strong>${count}</strong> students exported successfully.`,
        ...
        function handleStudentsImport(e) {
            ...
            formData.append('action', 'cbedu_import_students');
            formData.append('nonce', cbeduImportExport.importStudentsNonce);
            formData.append('cbedu_import_file', fileInput[0].files[0]);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        html += `<p><strong>✓ Created:</strong> ${data.imported} students</p>`;
                        html += `<p><strong>✓ Updated:</strong> ${data.updated} students</p>`;
